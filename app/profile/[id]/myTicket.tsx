...
import TrainDetails from '@/app/(mainPages)/train/components/trainDetails'
...
const TrainTicket = ({ticket}:ITrainTicket) => {
  return (
    
           <div className='capitalize bg-[#ffe0b9] p-4 rounded-md mt-8 w-full lg:w-10/12' >
          ...
          <h3 className="text-2xl mt-2 font-medium capitalize">
               ...
               <span className='px-2 text-secondary-100 capitalize'>
                 {
                   ticket.serviceType=="train"?"Railaways":"Metro"
                 }
               </span>
          </h3>
          ...
          <div className="ticket-content flex flex-col gap-8 lg:gap-0 justify-center lg:flex-row lg:justify-between items-center">
                 <div className="from-to w-full flex justify-between items-center">
                         <div className="from mt-4">
                               <h4 className='text-secondary-100 font-bold text-2xl'>{ticket.stations[0].time}</h4>
                               <h4 className='text-primary-100 font-bold text-2xl'>{ticket.stations[0].name}</h4>

                         </div>
                         <div className='destance flex flex-col items-center'>
                                {
                                ticket.serviceType=="train"?
                         ...
                         <div className="to mt-4">
                               <h4 className='text-secondary-100 font-bold text-2xl'>{ticket.stations[ticket.stations.length-1].time}</h4>
                               <h4 className='text-primary-100 font-bold text-2xl'>{ticket.stations[ticket.stations.length-1].name}</h4>
                 ...
                 <div className="count-persons w-full flex justify-evenly items-center ps-0 lg:ps-14">
                       <div className="count w-full flex justify-start items-center gap-1">
                             ...
                             <h4 className="count-number text-secontary-100">{ticket.count}</h4>
                             <h4 className="count-string text-secontary-100">.{NumberTransilation[ticket.count]}</h4>
                       </div>

                       <div className="price-section flex flex-col justify-center items-center gap-2 text-secondary-100">

                            <h4 className="price capitalize text-3xl flex justify-center items-center gap-2"><span>EGP</span> {ticket.price}</h4>
          ...
          <Accordion type="single" collapsible className='w-full' >
              <AccordionItem value='1' >
                    ...
                    <AccordionContent className='w-full' >
                          <TrainDetails  count={ticket.count} price={ticket.price} ticket={ticket}/>
                    </AccordionContent>
